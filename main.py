"""
╔══════════════════════════════════════════════════════════════════════════╗
║        🌌 COSMIC TAPPER v4 ULTIMATE — FastAPI Web App Backend          ║
║  Только FastAPI + aiosqlite, без Telegram Bot                           ║
╚══════════════════════════════════════════════════════════════════════════╝

УСТАНОВКА:
  pip install fastapi uvicorn aiosqlite aiofiles python-dotenv
...
# ─── ТАБЛИЦЫ БОНУСОВ ─────────────────────────────────────────────────────────
REBIRTH_BONUS  = {i: (1.45**i)*(1+i*0.18) if i else 1.0 for i in range(MAX_REBIRTH+1)}
PRESTIGE_BONUS = {i: (2.0**i)*(1+i*0.3)   if i else 1.0 for i in range(MAX_PRESTIGE+1)}

PRESTIGE_UNLOCKS = {
    1:"🔓 Автокликер I",      2:"🔓 Комбо",              3:"🔓 Крит-тап",
    5:"🔓 Пассивный доход I", 8:"🔓 Легендарные (H)",    10:"🔓 Квесты",
    15:"🔓 Пассивный доход II",20:"🔓 Вознесение",        25:"🔓 Омега (I)",
    30:"🔓 Месячные квесты",  35:"🔓 Экстрим (J)",        40:"🔓 Бесконечность (K)",
    50:"🔓 МегаСтак (L)",     60:"🔓 Берсерк (M)",        65:"🔓 Маска (N) + РЕЖИМ БОГА",
}
...
# ─── УЛУЧШЕНИЯ (14 веток × 150 уровней) ─────────────────────────────────────
_MAX_COST = 10**300

def _sc(base, growth, level):
    try:    v = base*(growth**level); return _MAX_COST if v>_MAX_COST else int(v)
    except: return _MAX_COST
...
# ─── ДОСТИЖЕНИЯ ──────────────────────────────────────────────────────────────
ACHIEVEMENTS = {
    "first_tap":   {"name":"Первый шаг",       "desc":"Первый тап",             "reward":100,       "cond":lambda p:p.get("taps",0)>=1},
    ...
    "earn_1k":     {"name":"Копилка",          "desc":"1 000 монет",            "reward":500,       "cond":lambda p:p.get("earned",0)>=1000},
    "earn_1m":     {"name":"Миллионер",        "desc":"1M монет",               "reward":10000,     "cond":lambda p:p.get("earned",0)>=1000000},
    "earn_1b":     {"name":"Миллиардер",       "desc":"1B монет",               "reward":1000000,   "cond":lambda p:p.get("earned",0)>=1000000000},
    "earn_1t":     {"name":"ТРИЛИОНЕР",        "desc":"1T монет",               "reward":50000000,  "cond":lambda p:p.get("earned",0)>=1000000000000},
    ...
    "prestige_max":{"name":"БОЖЕСТВЕННЫЙ",     "desc":f"{MAX_PRESTIGE} прест.", "reward":100000000, "cond":lambda p:p.get("prestige",0)>=MAX_PRESTIGE},
    "ascend_1":    {"name":"Вознесение",       "desc":"Первое вознесение",      "reward":5000000,   "cond":lambda p:p.get("ascension",0)>=1},
    "ascend_max":  {"name":"СОЗДАТЕЛЬ",        "desc":f"{MAX_ASCENSION} возн.", "reward":999999999, "cond":lambda p:p.get("ascension",0)>=MAX_ASCENSION},
    ...
    "boss_slain":  {"name":"Убийца боссов",    "desc":"Первый босс убит",       "reward":20000,     "cond":lambda p:p.get("bosses_killed",0)>=1},
    "boss_master": {"name":"Мастер боссов",    "desc":"10 боссов убито",        "reward":200000,    "cond":lambda p:p.get("bosses_killed",0)>=10},
    "clan_member": {"name":"Клановый воин",    "desc":"Вступить в клан",        "reward":10000,     "cond":lambda p:p.get("clan_id") is not None},
}

# ─── СОБЫТИЯ ─────────────────────────────────────────────────────────────────
EVENTS = [
    {"name":"☄️ Метеоритный дождь", "desc":"Все доходы ×10 на 30 мин!",        "tap_mult":10,"auto_mult":10,"passive_mult":10,"duration":1800},
    {"name":"⚡ Грозовой шторм",    "desc":"Мощь тапа ×5 на 15 мин!",          "tap_mult":5,                                 "duration":900},
    {"name":"💎 Алмазный дождь",    "desc":"Пассив. доход ×50 на 15 мин!",                       "passive_mult":50,          "duration":900},
    {"name":"🌙 Лунная власть",     "desc":"Офлайн доход ×100 на час!",                                        "offline_mult":100,"duration":3600},
    {"name":"🌀 Космический вихрь", "desc":"Автодоход ×25 на 20 мин!",                "auto_mult":25,                       "duration":1200},
    {"name":"🔮 Кристальный пульс", "desc":"Шанс крита 100% на 10 мин!",        "crit_override":1.0,                       "duration":600},
    {"name":"🌈 Радужная волна",    "desc":"Все доходы ×30 на 10 мин!",         "tap_mult":30,"auto_mult":30,"passive_mult":30,"duration":600},
    {"name":"💫 Звёздный шквал",   "desc":"Мощь тапа ×20, комбо фриз 5 мин!", "tap_mult":20,"combo_freeze":True,           "duration":300},
]
...
def calc_tap_power(p: dict) -> float:
    base = 1.0 + _sum(p, "tap_bonus")
    base *= REBIRTH_BONUS.get(min(p.get("rebirth", 0), MAX_REBIRTH), 1.0)
    base *= PRESTIGE_BONUS.get(min(p.get("prestige", 0), MAX_PRESTIGE), 1.0)
    asc = p.get("ascension", 0)
    if asc >= 1:
        key = max(k for k in ASCENSION_BONUSES if k <= asc)
        base *= ASCENSION_BONUSES[key][1]
    base *= _mult(p, "mult_bonus")
    if time.time() < p.get("event_end", 0):
        base *= p.get("event_tap_mult", 1.0)
    return base

def calc_auto(p: dict) -> float:
    t = _sum(p, "auto_bonus")
    if time.time() < p.get("event_end", 0):
        t *= p.get("event_auto_mult", 1.0)
    return t

def calc_passive(p: dict) -> float:
    t = _sum(p, "passive_bonus")
    if time.time() < p.get("event_end", 0):
        t *= p.get("event_passive_mult", 1.0)
    return t

def calc_income(p: dict) -> float:
    return calc_auto(p) + calc_passive(p)

def calc_crit_chance(p: dict) -> float:
    if time.time() < p.get("event_end", 0) and p.get("event_crit_override", -1) >= 0:
        return 1.0
    return min(_sum(p, "crit_bonus"), 0.95)

def calc_crit_mult(p: dict) -> float:
    t = 2.0 + _sum(p, "crit_mult")
    if p.get("ascension", 0) >= 20:
        t *= 999
    return t

def calc_combo_mult(p: dict) -> float:
    cb = _sum(p, "combo_bonus")
    c = p.get("combo", 0)
    return 1.0 if c <= 1 else 1.0 + cb * (c - 1)

def offline_income(p: dict) -> int:
    now = time.time()
    ...
    for k, lv in p.get("upgrades", {}).items():
        if lv <= 0: continue
        u = UPGRADES.get(k)
        if u and "offline_mult" in u:
            om *= u["offline_mult"] ** lv
    if time.time() < p.get("event_end", 0):
        om *= p.get("event_offline_mult", 1.0)
    income = int(calc_income(p) * om * delta)
    if income > 0:
        p["coins"] += income
        p["earned"] += income
    p["last_seen"] = now
...
def upgrade_cost(key: str, p: dict) -> int:
    ...
    if p.get("ascension", 0) >= 3:
        c = int(c * 0.3)
    return c

def rebirth_cost(rb):  return int(10000 * (4 ** rb))
def prestige_cost(pr): return 5 + pr * 2
def ascension_cost(a): return 20 + a * 5

def ensure_quests(p: dict):
    now = time.time()
    if not p.get("daily_quest"):
        p["daily_quest"] = random.choice(DAILY_QUESTS).copy()
        p["daily_progress"] = {}
        p["quest_reset_daily"] = now + 86400
    if not p.get("hourly_quest"):
        p["hourly_quest"] = random.choice(HOURLY_QUESTS).copy()
        p["hourly_progress"] = {}
        p["quest_reset_hourly"] = now + 3600
    if p.get("prestige", 0) >= 30 and not p.get("monthly_quest"):
        p["monthly_quest"] = random.choice(MONTHLY_QUESTS).copy()
        p["monthly_progress"] = {}
        p["quest_reset_monthly"] = now + 2592000

def update_quest(p: dict, prog_type: str, amount: int = 1):
    """Обновить прогресс квестов, вернуть список завершённых."""
    now = time.time()
    completed = []
    ...
    for qk, pk in [("daily_quest","daily_progress"),("hourly_quest","hourly_progress"),("monthly_quest","monthly_progress")]:
        q = p.get(qk)
        if not q or q["type"] != prog_type: continue
        pr = p.get(pk, {}); pr[prog_type] = pr.get(prog_type, 0) + amount; p[pk] = pr
        if pr[prog_type] >= q["target"]:
            p["coins"] += q["reward"]; p["earned"] += q["reward"]
            completed.append({"name": q["name"], "reward": q["reward"]})
            p[qk] = None
    return completed

def check_achievements(p: dict) -> list:
    """Проверить достижения, вернуть список новых."""
    new_ach = []
    for ach_id, ach in ACHIEVEMENTS.items():
        ...
        try:
            if ach["cond"](p):
                ...
                new_ach.append({"id": ach_id, "name": ach["name"], "reward": ach["reward"]})
        except: pass
    return new_ach

def fmt(n) -> str:
    n = float(n)
    for t, s in [(1e18,"Qi"),(1e15,"Q"),(1e12,"T"),(1e9,"B"),(1e6,"M"),(1e3,"K")]:
...
async def get_uid(request: Request) -> tuple[int, str]:
    """Извлекает user_id и username из запроса."""
    body = await request.json()
    init_data = body.get("initData", "")

    # DEV_MODE: принимаем _test_uid без проверки подписи
    if DEV_MODE:
...
async def lifespan(application: FastAPI):
    ...
    logger.info("🚀 Cosmic Tapper API запущен!")
    yield
    # Shutdown
    if DB:
        await DB.close()
...
app = FastAPI(title="Cosmic Tapper v4 API", lifespan=lifespan)
...
async def api_profile(request: Request):
    uid, uname = await get_uid(request)
    async with get_lock(uid):
        p = await load_player(uid)
        earned_offline = offline_income(p)
        ensure_quests(p)
    ...
    return {
        ...
        "bosses_killed": p.get("bosses_killed", 0), "combo": p.get("combo", 0),
        "tap_power": calc_tap_power(p), "income_per_sec": calc_income(p),
        "crit_chance": calc_crit_chance(p), "crit_mult": calc_crit_mult(p),
        "achievements": p.get("achievements", []), "achievements_total": len(ACHIEVEMENTS),
        ...
        "rebirth_cost": rebirth_cost(p["rebirth"]),
        "prestige_cost": prestige_cost(p["prestige"]),
        "ascension_cost": ascension_cost(p["ascension"]),
...
async def api_tap(request: Request):
    ...
    async with get_lock(uid):
        ...
        p["combo"] = (p.get("combo", 0) + 1) if (now - p["last_tap"] < 2.0 or freeze) else 1
        ...
        quests_done = update_quest(p, "taps")
        quests_done += update_quest(p, "earn", gain)
        if is_crit:
            quests_done += update_quest(p, "crits")
...
async def api_upgrades(request: Request):
    ...
    for branch_id, (emoji, name, prefix, req) in BRANCH_INFO.items():
        ...
        for k in keys[:20]:
            ...
            items.append({
                "key": k, "name": u["name"], "emoji": u["emoji"],
                "level": lv, "cost": c, "can_buy": p["coins"] >= c and unlocked,
            })
        result.append({
            "branch": branch_id, "emoji": emoji, "name": name,
...
async def api_buy_upgrade(request: Request):
    ...
    async with get_lock(uid):
        ...
        if p.get("prestige", 0) < u.get("prestige_req", 0):
            return {"ok": False, "msg": "Нет нужного престижа"}
        c = upgrade_cost(key, p)
        if p["coins"] < c:
            return {"ok": False, "msg": f"Нужно {fmt(c)} монет"}
        p["coins"] -= c
        p["upgrades"][key] = p.get("upgrades", {}).get(key, 0) + 1
        update_quest(p, "upgrade")
        new_ach = check_achievements(p)
        await save_player(uid, p, uname)
    return {"ok": True, "key": key, "level": p["upgrades"][key], "coins": p["coins"], "new_achievements": new_ach}
...
async def api_boss_attack(request: Request):
    ...
    async with get_lock(uid):
        ...
        if not boss:
            raise HTTPException(400, "Unknown boss")
        if p.get("prestige", 0) < boss.get("prestige_req", 0):
            return {"ok": False, "msg": f"Нужен Престиж {boss['prestige_req']}"}
        if p.get("boss_key") != boss_key:
            p["boss_key"] = boss_key; p["boss_hp"] = boss["hp"]
        ...
        if p["boss_hp"] <= 0:
            ...
            update_quest(p, "boss_kill")
            check_achievements(p)
        await save_player(uid, p, uname)
    return {
        "ok": True, "killed": killed, "damage": damage,
...
async def api_rebirth(request: Request):
    ...
    async with get_lock(uid):
        ...
        rb = p.get("rebirth", 0)
        if rb >= MAX_REBIRTH:
            return {"ok": False, "msg": "Максимум ребиртов!"}
        need = rebirth_cost(rb)
        if p["coins"] < need:
            return {"ok": False, "msg": f"Нужно {fmt(need)} монет"}
        p["coins"] = 0; p["upgrades"] = {}; p["combo"] = 0; p["rebirth"] = rb + 1
        update_quest(p, "rebirths"); check_achievements(p)
        await save_player(uid, p, uname)
    return {"ok": True, "rebirth": p["rebirth"], "bonus": REBIRTH_BONUS.get(p["rebirth"], 1.0)}
...
async def api_prestige(request: Request):
    ...
    async with get_lock(uid):
        ...
        pr = p.get("prestige", 0)
        if pr >= MAX_PRESTIGE:
            return {"ok": False, "msg": "Максимум престижей!"}
        need_rb = prestige_cost(pr)
        if p.get("rebirth", 0) < need_rb:
            return {"ok": False, "msg": f"Нужно {need_rb} ребиртов"}
        p["coins"] = 0; p["upgrades"] = {}; p["rebirth"] = 0; p["combo"] = 0; p["prestige"] = pr + 1
        unlock = PRESTIGE_UNLOCKS.get(p["prestige"], "")
        check_achievements(p); await save_player(uid, p, uname)
...
async def api_ascension(request: Request):
    ...
    async with get_lock(uid):
        ...
        if asc >= MAX_ASCENSION:
            return {"ok": False, "msg": "Максимум вознесений!"}
        if p.get("prestige", 0) < 20:
            return {"ok": False, "msg": "Нужен Престиж 20"}
        need_pr = ascension_cost(asc)
        if p.get("prestige", 0) < need_pr:
            return {"ok": False, "msg": f"Нужно {need_pr} престижей"}
...
async def api_clan_create(request: Request):
    ...
    if not name:
        raise HTTPException(400, "Нужно имя клана")
    async with get_lock(uid):
        p = await load_player(uid)
        if p.get("clan_id"):
            return {"ok": False, "msg": "Сначала покиньте текущий клан"}
        try:
            await DB.execute(
                "INSERT INTO clans(name, owner_id, members, created_at) VALUES(?,?,?,?)",
            ...
            return {"ok": False, "msg": "Имя занято или ошибка"}
    return {"ok": True, "clan_id": new_id}

@app.get("/api/bosses_list")
async def api_bosses_list():
    return [
        {"key": k, "name": v["name"], "hp": v["hp"], "reward": v["reward"],
         "emoji": v["emoji"], "prestige_req": v["prestige_req"]}
...
async def passive_loop():
    """Начислять авто+пассивный доход каждые 5 секунд всем игрокам."""
    while True:
        ...
        try:
            ...
            for uid, raw in rows:
                p = json.loads(raw)
                gain = int(calc_income(p) * 5)
                if gain > 0:
                    p["coins"] += gain; p["earned"] += gain
                    updates.append((json.dumps(p), uid))
            if updates:
                await DB.executemany("UPDATE players SET data=? WHERE user_id=?", updates)
                await DB.commit()
        except Exception as e:
            logger.error(f"[passive_loop] {e}")

async def event_loop():
    """Каждые 10 минут с 8% шансом выдавать событие активным игрокам."""
    while True:
        ...
        try:
            ...
            for uid, raw in rows:
                ...
                if random.random() < 0.08:
                    ...
                    p.update({
                        "event_end":          end,
                        "event_tap_mult":     ev.get("tap_mult", 1.0),
            ...
            logger.error(f"[event_loop] {e}")

# ─── СТАТИЧЕСКИЕ ФАЙЛЫ (фронтенд) ────────────────────────────────────────────
# index.html должен лежать рядом с main.py
static_dir = BASE_DIR
if (static_dir / "index.html").exists():
    app.mount("/", StaticFiles(directory=str(static_dir), html=True), name="static")
    logger.info(f"📁 Фронтенд раздаётся из {static_dir}")
else:
    logger.warning("⚠️  index.html не найден рядом с main.py — фронтенд не будет раздаваться")
